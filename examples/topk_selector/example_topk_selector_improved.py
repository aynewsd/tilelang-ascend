"""Optimized topk_selector: eliminate scalar copy loop, add ping-pong pipeline.

Key optimizations vs example_topk_selector.py:
1. Replace scalar element-copy loop (for i in range(block_N)) with single T.copy
   GM -> UB per row, eliminating the SCALAR BOUND bottleneck (47.4% -> ~0%).
2. Merge x_ub and score_accum into a single UB buffer (no intermediate).
3. Add ping-pong double buffering to overlap MTE2 (data load) with VEC (topk sort).
4. Use Expert+Developer hybrid mode: explicit T.copy + T.tile.topk intrinsics
   with auto-sync pass_configs for simplified barrier management.

Mode: Expert + Developer hybrid
  - Explicit T.copy for GM<->UB搬运 (Expert-style memory management)
  - pass_configs auto-sync enabled (Developer-style convenience)
"""
import argparse
...
def simple_topk_selector_improved(
    B: int, N: int, top_k: int, dtype: Literal["float32"] = "float32"
):
    VEC_NUM = 2
    INDEX_DTYPE = "int32"
    ...
    def main(
        ...
        with T.Kernel(b_num, is_npu=True) as (cid, vid):
            ...
            T.copy(x[row_id, 0:N], score_accum)
...
def check_case(B: int, N: int, top_k: int):
    ...
    kernel = simple_topk_selector_improved(B, N, top_k)
...
def main(custom_args=None):
    parser = argparse.ArgumentParser(description="topk_selector Improved Example")
    ...
    print("topk_selector improved example passed!")
